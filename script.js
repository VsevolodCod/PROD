...
document.getElementById("questionForm").onsubmit = function (event) {
  ...
  let hasCorrectAnswer = false; // Флаг для проверки наличия правильного ответа
  ...
  if (!testTitle.trim()) {
      isValid = false;
      document.getElementById("test-title").style.background = "#ffb6b6"; 
      document.getElementById("error-message").textContent += "Название теста не должно быть пустым. ";
  } else {
      document.getElementById("test-title").style.background = ""; // Сброс цвета рамки
  }
  ...
  questions.forEach((question, index) => {
      ...
      const answersData = [];
      let hasAnswer = false; // Флаг для проверки наличия ответа

      // Проверка на пустой вопрос
      if (!questionText.trim()) {
          isValid = false;
          question.querySelector("input[type='text']").style.background = "#ffb6b6"; // Красная рамка для пустого вопроса
          document.getElementById("error-message").textContent += `Вопрос ${index + 1} не должен быть пустым. `;
      } else {
          question.querySelector("input[type='text']").style.background = ""; // Сброс цвета рамки
      }
      ...
      answers.forEach((answer, optionIndex) => {
          ...
          if (!answerText.trim()) {
              isValid = false;
              answer.querySelector("input[type='text']").style.background = "#ffb6b6"; // Красная рамка для пустого ответа
              document.getElementById("error-message").textContent += `Ответ ${optionIndex + 1} для вопроса ${index + 1} не должен быть пустым. `;
          } else {
              answer.querySelector("input[type='text']").style.background = ""; // Сброс цвета рамки
          }

          // Проверка на наличие правильного ответа
          if (isCorrect) {
              hasAnswer = true; // Если есть правильный ответ
          }
      });

      if (hasAnswer) {
          hasCorrectAnswer = true; // Если есть хотя бы один правильный ответ
      }
  ...
  if (!hasCorrectAnswer) {
      document.getElementById("error-message").textContent += "Выберите правильный ответ в созданных вопросах.";
      return; // Просто возвращаем, не отправляя данные
  }
